...
BOX_EXPAND_H    = 1.6     # 高度放大倍数(模型框偏扁, 高度要多放 ~1.1 倍: 1.25*1.1)
...
def merge_bag_edge_strips(img, s, roi, gnd_boxes_for_edge=None, draw_debug=True):
    ...
    for eroi in rois:
        ...
        if (abs(lab[1] - s[7]) > max_delta
            ...
            continue
        if try_merge_bag_edge_stat(s, st):
            if not draw_debug:
                continue
...
def sample_det(img, det, draw_debug=True):
    """对(修正后)模型框中心正方形取样, 返回统计 9 元组; 附带取样区画框。"""
    label, score, x, y, w, h = det
...
def _gnd_thr_for_blobs():
    return (clamp_int(int(gnd_box[0]), 0, 100), clamp_int(int(gnd_box[1]), 0, 100),
            clamp_int(int(gnd_box[2]), -128, 127), clamp_int(int(gnd_box[3]), -128, 127),
            clamp_int(int(gnd_box[4]), -128, 127), clamp_int(int(gnd_box[5]), -128, 127))
...
def update_cut_line(img):
    global cut_left_y, cut_valid, cut_miss
    ...
    if valid_strips >= CUT_MIN_VALID_STRIPS:
        ...
        if not cut_valid:
            ...
            cut_left_y = int(CUT_EMA_ALPHA * top_y_min + (1.0 - CUT_EMA_ALPHA) * cut_left_y)
        cut_left_y = clamp_int(cut_left_y, 20, CUT_SCAN_Y_MAX)
    else:
        cut_miss += 1
    ...
    while len(samples) < SAMPLE_FRAMES:
        ...
        if exclude_slots and done_hint_slot is not None:
            if reject_draw is not None:
                draw_model_box(img, reject_draw, ok=False)
            swap_clear_count = 0
            brn_bear_stable_box = None
            brn_bear_stable_count = 0
            red_bag_stable_box = None
        ...
        if exclude_slots and swap_clear_count < SWAP_CLEAR_FRAMES:
            if reject_draw is not None:
                ...
                wait_pick = max(candidates, key=lambda it: (
                    collect_slot_priority(it[0], exclude_slots, allowed_slots), it[2]))
                draw_model_box(img, wait_pick[8], ok=False)
            swap_clear_count += 1
            brn_bear_stable_box = None
            brn_bear_stable_count = 0
            red_bag_stable_box = None
...
# ---------- 动态分界线(移植 xuezhang/另一个/main.py update_dynamic_cut, 阈值用近地面盒) ----------
# 水平裁切线: 5 条竖带取"从底部向上连续的布"的最高点; 贴底种子段(≥12px)+单次桥接(≤20px)
# 防噪点/黄线切断; ≥2 带有效; 双向对称 EMA。检测 ROI 从线以下开始(赛道外整体切掉)。
CUT_BLOB_MIN_H = 12
CUT_BLOB_BOTTOM_MARGIN = 25
CUT_GAP_BRIDGE = 20
CUT_STRIP_XS = (10, 85, 160, 235, 310)
...
def update_cut_line(img):
    # 与 xuezhang/另一个/main.py 同构: 5 带取最高点, >=2 带有效, 双向对称 EMA
    global cut_left_y, cut_valid, cut_miss
    thr = _gnd_thr_for_blobs()
    strip_h = CUT_SCAN_Y_MAX - CUT_SCAN_Y_MIN
    top_y_min = None
...
def dynamic_detect_roi():
    if cut_valid:
        y_base = clamp_int(cut_left_y - CUT_Y_MARGIN, 20, 220)
    else:
        y_base = 20
...
while True:
    ...
    if frame % 30 == 0:
        if best and model_dets:
            ...
            print("fps=%.1f [cmp] 色块%s%s 模型%s(%.2f)%s -> %s" % (
                clock.fps(), SLOT_NAMES[best[0]], bbox,
                LABEL_NAMES.get(md[0], "?"), md[1], mbox, tag))
        elif best and not model_dets:
            print("fps=%.1f [cmp] 色块%s%s 模型无检出 -> 疑似色块误报" % (
                clock.fps(), SLOT_NAMES[best[0]], best[1].rect()))
