...
YELLOW_ROI_LEFT = (0, 60, 40, 120)      # 左侧竖条检测区域
YELLOW_ROI_RIGHT = (280, 60, 40, 120)    # 右侧竖条检测区域
YELLOW_DETECT_INTERVAL = 5              # 每5帧检测一次黄线
YELLOW_ENTER_PIXELS = 10                # 首次看到黄线需要达到的像素阈值
YELLOW_KEEP_PIXELS = 7                  # 已看到黄线后的保持阈值，防止边缘闪烁
...
while True:
    ...
    if frame_count % YELLOW_DETECT_INTERVAL == 0:
        ...
        # 首次进入用较高阈值，看到后切到较低保持阈值，减少边缘像素闪烁导致的误丢失。
        if raw_yellow_seen:
