...
运行时必须提供 `/sd/80lite0.5SS.tflite`。`main` 分支通过 `.gitignore` 排除了 `*.tflite`，最终源码发布树不附带模型。请使用 label 顺序符合“目标定义”一节的模型，并只分发自己有权使用和再许可的模型。

历史分支 `archive/provincial-final-2026-07-21` 曾保存比赛时的模型快照。公开包含完整 Git 历史的仓库前，请先确认该模型的再分发权；MIT License 默认只覆盖本仓库源码和文档，不自动覆盖模型权重。
