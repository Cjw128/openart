...
- 白熊检测、颜色目标检测、黄线状态、UART 协议、IPM 和主循环都在对应的单文件主程序内；当前两车均不负责视觉回库。
...
变更：
- IPM 只描述地面点，因此主车和从车都改用目标外接框底边中点作为接地点，不再平均四个角点。目标上边缘接近或越过投影地平线时，不会再污染最终距离。
- 纵向世界坐标的有效输出范围改为 `0 < world_y <= 300 cm`；零值、负值、NaN、无穷值及超过上限的结果统一回传远距饱和值 `300 cm`。该值用于保证车辆继续向目标靠近，不表示 `300 cm` 处仍具有标定精度。
- 横向世界坐标继续限制在 `-250 cm` 至 `250 cm`。世界坐标写入 UART 前增加有符号 16 位饱和保护，避免超范围正数经过按位掩码后被主控解码成负数。
- 16 字节世界坐标协议格式和毫米单位保持不变；纵向远距饱和值在线路上为 `3000 mm`。

验证：
- `main.py` 和 `minimain.py` 的主机端语法编译已通过。
- 沿图像纵轴扫描代表性接地点后，两份程序的纵向结果均保持为正数且不超过 `300 cm`；`main.py` 跨过投影地平线后稳定保持 `300 cm`。
- 零值、负值、NaN、正负无穷值注入测试通过，均转换为 `300 cm`。
- 两份程序的远距 UART 数据包解码结果均为 `3000 mm`，超出 `int16` 的正负测试值均正确饱和。
- `git diff --check` 已通过。
