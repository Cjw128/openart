...
- 模型权重不公开，所有公开分支和标签均排除 `*.tflite`；旧省赛归档已重写为只保留脚本和配置，模型文件及其校验值不再出现在可达 Git 历史中。
...
- 模型刷新帧在模型框左右各内缩 `5%`、顶部内缩 `5%`、底部内缩 `10%` 的区域重新找颜色，并忽略旧颜色框引用；模型拥有显示 / 坐标几何，颜色框只用包围框中心位移带动两者，不再用颜色框尺寸直接改变距离几何。
- 显示框和坐标框恢复省赛冻结版的同一单级跟随：当前值 `70%`、上一值 `30%`，框中心位移超过 `36 px` 时当帧直接采用新框。删除后加的独立 `50%` 坐标 EMA、`2 px` 接触点死区、`18 px` 跳变旁路以及未提交的近距离降权 / 双帧确认实验。
...
- 普通自由搜索首锁由 `5/7` 调整为 `3/5`，中心连续性容差由 `24 px` 放宽至 `36 px`，尺寸变化容差由 `35%` 放宽至 `50%`；首锁置信度保持 `0.30`，曝光和颜色 IQR 不变。主控指定颜色原本已使用同等 `3/5` 容错。
- `test_model_blob_fusion.py` 从 13 项增至 18 项，新增运动首锁策略、开关回退、半径门控、序号身份、负坐标解码、连续帧边界和坏校验重同步覆盖。
